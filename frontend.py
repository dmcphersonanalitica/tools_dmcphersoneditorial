...
st.set_page_config(page_title='Transcription', page_icon='static_file/image/favicon.ico')
...
st.title('DMcPherson Editorial Transcription')
...
if file is not None and transcription_type == 'Audio':
    ...
    if transcribe:
        try:
            ...
            model = back.load_model(transcription_quality)
            if model:
                st.success('The transcription model has been loaded')
                with st.spinner('Transcribing...'):
                    transcription = back.audio_transcription(model, file_path, transcription_format)
